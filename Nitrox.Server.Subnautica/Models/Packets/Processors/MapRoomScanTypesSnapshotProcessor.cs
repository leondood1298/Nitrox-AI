...
internal sealed class MapRoomScanTypesSnapshotProcessor(EntityRegistry entityRegistry, SimulationOwnershipData simulationOwnershipData, ScannerRoomDiagnostics diagnostics) : IAuthPacketProcessor<MapRoomScanTypesSnapshot>
{
    public async Task Process(AuthProcessorContext context, MapRoomScanTypesSnapshot packet)
    {
        if (!entityRegistry.TryGetEntityById(packet.MapRoomId, out MapRoomEntity room))
        {
            ...
            await context.ReplyAsync(new MapRoomScanTypesSnapshot(packet.MapRoomId, [], [], packet.ScanOrigin, packet.ScanRange, 0, true, false));
            return;
        }
        ...
        {
            diagnostics.RecordRejected("scan_types", room, sessionId: context.Sender.SessionId, reason: "invalid_anchor");
            await context.ReplyAsync(new MapRoomScanTypesSnapshot(packet.MapRoomId, [], [], packet.ScanOrigin, packet.ScanRange, 0, true, false));
            return;
        }
        if (!MapRoomScanTypesAuthority.TryApply(room, packet, entityRegistry.GetEntities<WorldEntity>(), scanAnchor, out List<NitroxTechType> accepted, out long revision))
        {
        ...
        await context.SendToAllAsync(new MapRoomScanTypesSnapshot(packet.MapRoomId, accepted, [], packet.ScanOrigin, packet.ScanRange, revision, true, true));
    }
}
