...
| D0 | 2 | With both clients connected, finish a new Scanner Room on an existing base; before any relog, the non-builder controls and moves each default drone while the builder watches, then picks up/drops/redocks both | Both clients bind the same two canonical IDs immediately; control/movement is visible both ways; pickup/drop preserves identity; no extra camera entity is created | NOT RUN |
